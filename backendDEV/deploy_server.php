...
function deploy($package, $version, $target)
{
	switch($target){
		...
		case "frontend":
			...
			//scp package over to QA backend
...
function deploy_fail($package, $version, $target)
{
		...
		case "frontend":
			//set the passfail to passed 
			$stmt = "UPDATE `frontend` SET `passfail` = 'fail' WHERE `package` = :package AND `version` = :version";
			$params = array(":package" => $package, ":version" => $version);
			//send to database
			$result = send_sql_query_to_databse(true,$stmt,$params);
			return;
		...
		case "backend":
			//set the passfail to passed 
			$stmt = "UPDATE `backend` SET `passfail` = 'fail' WHERE `package` = :package AND `version` = :version";
			$params = array(":package" => $package, ":version" => $version);
			//send to database
			$result = send_sql_query_to_databse(true,$stmt,$params);
			return;
